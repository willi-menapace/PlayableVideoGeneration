...
class DetectionMetric1D():
    ...
    def __call__(self, reference_detections: np.ndarray, generated_detections: np.ndarray, prefix: str) -> Dict:
        '''
        Computes the mean error between the reference and the generated observations

        :param reference_detections: (sequences_count, observations_count) tensor with detections, -1 if detection is missing
        :param generated_detections: (sequences_count, observations_count) tensor with detections, -1 if detection is missing
        ...
        for sequence_idx in range(sequences_count):
            ...
            for observation_idx in range(sequence_length):
                reference_successful = reference_detections[sequence_idx, observation_idx] != -1
                generated_successful = generated_detections[sequence_idx, observation_idx] != -1
                ...
                if reference_successful and generated_successful:
                    ...
                    center_distances[observation_idx] += np.abs(reference_detections[sequence_idx, observation_idx] - generated_detections[sequence_idx, observation_idx])
